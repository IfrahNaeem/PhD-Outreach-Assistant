...
def fetch_page_text(url):
    ...
    try:
        ...
        # Many faculty pages show a "Contact" link or an icon whose visible
        # text isn't the email — the real address is hidden in the link's
        # mailto: href. Surface it inline as visible text so the extraction
        # step downstream can actually see it in context.
        for link in soup.find_all("a", href=True):
            href = link["href"]
            if href.lower().startswith("mailto:"):
                email = href.split(":", 1)[1].split("?")[0].strip()
                visible = link.get_text(strip=True)
                if email and email.lower() not in visible.lower():
                    link.append(f" ({email})")
