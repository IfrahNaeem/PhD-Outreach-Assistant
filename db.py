...
from sqlalchemy import (
    create_engine, Column, Integer, String, Text, DateTime, ForeignKey, LargeBinary
)
...
def save_applicant_profile(user_id, your_name, target_degree, background):
    s = get_session()
    try:
...
def get_applicant(user_id):
    ...
    try:
        u = s.get(User, user_id)
        if u is None:
            return {"your_name": "", "target_degree": "", "background": "",
