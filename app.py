...
# =============================================================================
    ...
    if prof_opts:
        st.markdown("#### Delete a professor")
        st.caption("Removes the professor and any drafts/history tied to them. This can't be undone.")
        del_label = st.selectbox("Professor to delete", list(prof_opts.keys()), key="delete_prof_select")
        if st.button("🗑️ Delete Professor"):
            ok, msg = db.delete_professor(user_id, prof_opts[del_label])
            (st.success if ok else st.error)(msg)
            if ok:
                st.rerun()
