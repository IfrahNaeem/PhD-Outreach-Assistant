...
# =============================================================================
    ...
    with st.container(border=True):
        st.markdown("#### 🌐 Extract from a faculty page URL")
        st.caption(
            "Paste the exact URL of a department's faculty/people page. The app fetches that one "
            "page and asks the AI to pull out names, research areas, and any emails that are "
            "actually visible on it — nothing invented, nothing else on the site gets touched. "
            "Review the results before adding anyone."
        )
        ...
        if st.button("Fetch & Extract"):
            ...
            if not ok:
                ...
                if not ok2:
                    ...
                    st.warning("Nothing that looked like a faculty directory was found on that page.")
                else:
