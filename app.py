    ...
    if not options:
        ...
        with st.container(border=True):
            ...
            st.caption(
                ...
                "click. Each is still written individually from that professor's own research — this "
                "just saves you clicking one at a time. Every email still needs its own separate "
                "approval before it can ever be sent."
            )
            ...
            if st.button("Generate for Selected", type="primary", disabled=not selected_labels):
                ...
                for i, sel_label in enumerate(selected_labels):
                    ...
                    research = db.get_research(user_id, pid)
                    progress.progress(i / total, text=f"Generating for {prof['professor_name']}...")

                    if not research:
                        results.append((prof["professor_name"], False,
                                         "Skipped — no research yet. Run Research & Fit for this professor first."))
                        continue
                    ...
                    db.new_message(user_id, pid, "EMAIL", data.get("subject", ""), body_html, body_text)
                    ...
                    if include_followups:
                        ...
                        if ok2:
                            for key, ftype in [("day7", "FOLLOW_UP_DAY7"), ("day14", "FOLLOW_UP_DAY14")]:
                                bt = data2.get(key, "")
                                bh = "".join(f"<p>{line}</p>" for line in bt.split("\n") if line.strip())
                                db.new_message(user_id, pid, ftype, "", bh, bt)
                            status_msg = "Email + follow-ups generated."
                        else:
                            status_msg = f"Email generated, but follow-ups failed: {msg2}"
